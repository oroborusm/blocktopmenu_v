...
require (dirname(__FILE__).'/MenuTopLinks_v.class.php');
/* Se modifica el nombre del menu*/
class Blocktopmenu_v extends Module
{
	private $_menu = '';
	private $_html = '';
	private $user_groups;
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Top vertical menu');
	...
	public function install($delete_params = true)
	{
		...
		if ($delete_params)
			if (!$this->installDb() || !Configuration::updateGlobalValue('MOD_BLOCKTOPMENU_V_ITEMS', 'CAT3,CAT26') || !Configuration::updateGlobalValue('MOD_BLOCKTOPMENU_SEARCH', '1'))
				return false;
	...
	public function installDb()
	{
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'linksmenutop_v` (
			`id_linksmenutop_v` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`new_window` TINYINT( 1 ) NOT NULL,
			INDEX (`id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;') &&
			Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'linksmenutop_v_lang` (
			`id_linksmenutop_v` INT(11) UNSIGNED NOT NULL,
			`id_lang` INT(11) UNSIGNED NOT NULL,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`label` VARCHAR( 128 ) NOT NULL ,
			`link` VARCHAR( 128 ) NOT NULL ,
			INDEX ( `id_linksmenutop_v` , `id_lang`, `id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;'));
	}
	...
	public function uninstall($delete_params = true)
	{
		...
		if ($delete_params)
			if (!$this->uninstallDB() || !Configuration::deleteByName('MOD_BLOCKTOPMENU_V_ITEMS') || !Configuration::deleteByName('MOD_BLOCKTOPMENU_SEARCH'))
				return false;
	...
	{
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'linksmenutop_v`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'linksmenutop_v_lang`');
		return true;
	}
	...
	public function getContent()
	{
			...
			foreach ($shops as $shop_id)
			{
				...
				if (count($shops) == 1)
				{
					if (is_array($items) && count($items))
		 				$updated = Configuration::updateValue('MOD_BLOCKTOPMENU_V_ITEMS', (string)implode(',', $items), false, (int)$shop_group_id, (int)$shop_id);
		 			else
		 				$updated = Configuration::updateValue('MOD_BLOCKTOPMENU_V_ITEMS', '', false, (int)$shop_group_id, (int)$shop_id);
		 		}
		...
		else if (Tools::isSubmit('submitBlocktopmenuLinks'))
		{
			...
			if ($count_links_label || $count_label)
			{
				...
				else
				{
					...
					foreach ($shops as $shop_id)
					{
						$added = MenuTopLinks_v::add($links_label, $labels,  Tools::getValue('new_window', 0), (int)$shop_id);
		...
		elseif (Tools::isSubmit('deletelinksmenutop_v'))
		{
			$errors_delete_link = array();
			$id_linksmenutop_v = Tools::getValue('id_linksmenutop_v', 0);
			$shops = Shop::getContextListShopID();

			foreach ($shops as $shop_id)
			{
				$deleted = MenuTopLinks_v::remove($id_linksmenutop_v, (int)$shop_id);
				Configuration::updateValue('MOD_BLOCKTOPMENU_V_ITEMS', str_replace(array('LNK'.$id_linksmenutop_v.',', 'LNK'.$id_linksmenutop_v), '', Configuration::get('MOD_BLOCKTOPMENU_V_ITEMS')));
		...
		elseif (Tools::isSubmit('updatelinksmenutop_v'))
		{
			$id_linksmenutop_v = (int)Tools::getValue('id_linksmenutop_v', 0);
			...
			if (Tools::isSubmit('updatelink'))
			{
				...
				MenuTopLinks_v::update($link, $label, $new_window, (int)$id_shop, (int)$id_linksmenutop_v, (int)$id_linksmenutop_v);
		...
		foreach ($shops as $shop_id)
			$links = array_merge($links, MenuTopLinks_v::gets((int)$id_lang, null, (int)$shop_id));
	...
	private function getMenuItems()
	{
		...
		else
		{
			...
			if (count($shops) > 1)
			{
				...
				{
					$shop_group_id = Shop::getGroupFromShop($shop_id);
					$conf .= (string)($key > 1 ? ',' : '').Configuration::get('MOD_BLOCKTOPMENU_V_ITEMS', null, $shop_group_id, $shop_id);
				}
			}
			else
			{
				$shop_id = (int)$shops[0];
				$shop_group_id = Shop::getGroupFromShop($shop_id);
				$conf = Configuration::get('MOD_BLOCKTOPMENU_V_ITEMS', null, $shop_group_id, $shop_id);
			}
	...
	private function makeMenuOption()
	{
		...
		foreach ($menu_item as $item)
		{
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					$link = MenuTopLinks_v::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						if (!isset($link[0]['label']) || ($link[0]['label'] == ''))
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = MenuTopLinks_v::get($link[0]['id_linksmenutop_v'], (int)$default_language, (int)Shop::getContextShopID());
						}
						$html .= '<option selected="selected" value="LNK'.(int)$link[0]['id_linksmenutop_v'].'">'.Tools::safeOutput($link[0]['label']).'</option>';
					}
					break;
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = MenuTopLinks_v::get($link[0]['id_linksmenutop_v'], $default_language, (int)Shop::getContextShopID());
						}
	...
	public function hookDisplayTopColumn($param)
	{
		...
		$html = $this->display(__FILE__, 'blocktopmenu_v.tpl', $this->getCacheId());
		return $html;
	}
	...
	{
		$this->_clearCache('blocktopmenu_v.tpl');
	}

	public function hookActionShopDataDuplication($params)
	{
		$linksmenutop_v = Db::getInstance()->executeS('
			SELECT *
			FROM '._DB_PREFIX_.'linksmenutop_v
			WHERE id_shop = '.(int)$params['old_id_shop']
			);

		foreach($linksmenutop_v as $id => $link)
		{
			Db::getInstance()->execute('
				INSERT IGNORE INTO '._DB_PREFIX_.'linksmenutop_v (id_linksmenutop_v, id_shop, new_window)
				VALUES (null, '.(int)$params['new_id_shop'].', '.(int)$link['new_window'].')');

			$linksmenutop_v[$id]['new_id_linksmenutop_v'] = Db::getInstance()->Insert_ID();
		}
		...
		foreach($linksmenutop_v as $id => $link)
		{
			$lang = Db::getInstance()->executeS('
					...
					FROM '._DB_PREFIX_.'linksmenutop_v_lang
					WHERE id_linksmenutop_v = '.(int)$link['id_linksmenutop_v'].' AND id_shop = '.(int)$params['old_id_shop']);

			foreach($lang as $l)
				Db::getInstance()->execute('
					INSERT IGNORE INTO '._DB_PREFIX_.'linksmenutop_v_lang (id_linksmenutop_v, id_lang, id_shop, label, link)
					VALUES ('.(int)$link['new_id_linksmenutop_v'].', '.(int)$l['id_lang'].', '.(int)$params['new_id_shop'].', '.(int)$l['label'].', '.(int)$l['link'].' )');
		}
	}
	...
	public function renderAddForm()
	{
		$fields_form = array(
			'form' => array(
				'legend' => array(
					'title' => (Tools::getIsset('updatelinksmenutop_v') && !Tools::getValue('updatelinksmenutop_v')) ? $this->l('Update link') : $this->l('Add a new link'),
					'icon' => 'icon-link'
				),
				'input' => array(
					array(
		...
		if (Tools::getIsset('updatelinksmenutop_v') && !Tools::getValue('updatelinksmenutop_v'))
			$fields_form['form']['submit'] = array(
				'name' => 'updatelinksmenutop_v',
				'title' => $this->l('Update')
			);

		if (Tools::isSubmit('updatelinksmenutop_v'))
		{
			$fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'updatelink');
			$fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_linksmenutop_v');
	...
	public function renderChoicesSelect()
	{
		...
		$links = MenuTopLinks_v::gets($this->context->language->id, null, (int)Shop::getContextShopID());
		foreach ($links as $link)
		{
			if ($link['label'] == '')
			{
				$default_language = Configuration::get('PS_LANG_DEFAULT');
				$link = MenuTopLinks_v::get($link['id_linksmenutop_v'], $default_language, (int)Shop::getContextShopID());
				if (!in_array('LNK'.(int)$link[0]['id_linksmenutop_v'], $items))
					$html .= '<option value="LNK'.(int)$link[0]['id_linksmenutop_v'].'">'.$spacer.Tools::safeOutput($link[0]['label']).'</option>';
			}
			elseif (!in_array('LNK'.(int)$link['id_linksmenutop_v'], $items))
				$html .= '<option value="LNK'.(int)$link['id_linksmenutop_v'].'">'.$spacer.Tools::safeOutput($link['label']).'</option>';
		}
		$html .= '</optgroup>';
		$html .= '</select>';
		return $html;
	...
	public function getAddLinkFieldsValues()
	{
		...
		if (Tools::isSubmit('updatelinksmenutop_v'))
		{
			$link = MenuTopLinks_v::getLinkLang(Tools::getValue('id_linksmenutop_v'), (int)Shop::getContextShopID());
		...
		$fields_values = array(
			...
			'id_linksmenutop_v' => Tools::getValue('id_linksmenutop_v'),
		);
	...
	public function renderList()
	{
		...
		foreach ($shops as $shop_id)
			$links = array_merge($links,  MenuTopLinks_v::gets((int)$this->context->language->id, null, (int)$shop_id));

		$fields_list = array(
			'id_linksmenutop_v' => array(
		...
		$helper->identifier = 'id_linksmenutop_v';
		$helper->table = 'linksmenutop_v';
